...
(() => {
  ...
  tabs.forEach((t, i) => {
    t.addEventListener('click', () => setMode(t.dataset.lpMode));
  ...
  $$('[data-lp-model]').forEach(b => b.addEventListener('click', () => setModel(b.dataset.lpModel)));
  ...
  function barCheck() {
    ...
    if (!phone.matches) { barEl.hidden = true; return; }
  ...
  $$('.lp .lp-seals, [data-lp-steps], [data-lp-years], [data-lp-map]').forEach(el => reduce ? el.classList.add('is-in') : io.observe(el));
  
  addEventListener('scroll', () => { $$('.lp .lp-seals:not(.is-in), [data-lp-steps]:not(.is-in), [data-lp-years]:not(.is-in), [data-lp-map]:not(.is-in)').forEach(el => { if (el.getBoundingClientRect().bottom < 0) el.classList.add('is-in'); }); }, { passive: true });
  ...
  const form = (() => {
    ...
    sysBtns.forEach(b => b.addEventListener('click', () => { pick(b.dataset.v); window.ffyTrack && window.ffyTrack('system_select', { system, form: 'location' }); setTimeout(() => show(2), 180); }));
